...
#Takes (high-low)/2 avg price, multiplies by volume, and finds that average
def getVWAP(tickerObj, price):
    total = ((tickerObj.iloc[:, 1] + tickerObj.iloc[:, 2])/2) * tickerObj["Volume"]
...
# Sharpe Ratio
# Formula is (rp-rf)/stdDev, return of portfolio is the average daily returns
def getSharpeRatio(tickerObj, price, avg = True):
    #rf = 0.011
    drp = getDailyReturnPercentage(tickerObj, price)
    rp = drp.mean()
    stdDev = drp.std()
    return (rp / stdDev)

def getDailyReturnPercentage(tickerObj, price):
    stock = tickerObj
    newCol = np.zeros(len(stock))
    for i in range(1, len(stock)):
        avgprice2 = (stock.iloc[i, 1] + stock.iloc[i, 2])/2 + stock.iloc[i, 5]
        avgprice1 = (stock.iloc[i - 1, 1] + stock.iloc[i - 1, 2])/2 + stock.iloc[i - 1, 5]
        percentage = (avgprice2-avgprice1)/avgprice1
        newCol[i] = percentage
    return newCol[1:]
...
def upThreeDownThree(tickers):
    yfObj = getTickers(tickers)
    tickerPriceHist = []
    
    for stock in yfObj:
        allHistory = stock.history()
        newDF = allHistory[["High", "Low"]]
        newDF['AvgPrice'] = ((newDF['High'] + newDF["Low"])/2)
        tickerPriceHist.append(newDF)
    
    
    
    return tickerPriceHist
